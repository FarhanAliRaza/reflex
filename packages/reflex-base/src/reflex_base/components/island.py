...
The Astro compiler auto-places islands using tree signals and component
metadata. `rx.island(...)` is used only to:
...
actual `client:*` directive emission happens in the Astro page emitter. On the
React Router target the wrapper is a no-op.
"""
...
_VALID_HYDRATE_STRATEGIES: tuple[HydrateStrategy, ...] = get_args(HydrateStrategy)
...
def _validate_hydrate(hydrate: Any) -> HydrateOption:
    ...
    if isinstance(hydrate, str):
        if hydrate in _VALID_HYDRATE_STRATEGIES:
            return hydrate
        msg = (
