...
def find_static_mode_violations(
    *,
    ...
    for node in _iter_descendants(root):
        ...
        # Class-level hydration metadata.
        if (
            ...
            violations.append(
                StaticModeViolation(
                    ...
                    detail=(
                        "client_only="
                        f"{bool(getattr(node, 'client_only', False))}, "
                        "requires_hydration="
                        f"{bool(getattr(node, 'requires_hydration', False))}, "
                        "provides_hydrated_context="
                        f"{bool(getattr(node, 'provides_hydrated_context', False))}"
                    ),
                )
            )
            continue
