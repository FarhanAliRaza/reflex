...
This module implements the deterministic island-boundary placement rule from
``ASTRO_MIGRATION_TASKS.md`` Master Task 2:
...
def _has_state_signal(node: Any) -> bool:
    ...
    The check mirrors ``MemoizeStatefulPlugin._should_memoize``'s direct
    signal detection: own props with state-bound ``VarData`` or any event
    trigger registered on the component.
...
def _placement_for_island_wrapper(
    ...
    if spec.client_only:
        ...
        if spec.hydrate == "load":
            directive = "client:load"
        elif spec.hydrate == "idle":
