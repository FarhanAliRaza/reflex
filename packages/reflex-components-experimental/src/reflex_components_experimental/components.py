"""Static, presentational components: plain HTML elements styled with atomic
Tailwind utilities against Radix's exact design tokens (shipped in ``theme.css``).
...
These cover the non-interactive surface (typography, layout, cards, badges,
inputs, ...). Interactive widgets that need ARIA/keyboard behavior live in
:mod:`reflex_components_experimental.interactive`, where the same token styling
is layered on Base UI headless parts.
"""
