...
1. **Ship Radix's exact tokens** (space, font-size,
...
Run: dev server (`reflex run`) + `uv run python diff.py` (set `BUISPIKE_URL`
to override the default `http://localhost:3000`, e.g. when running the app on
another port). Tokens come solely from `ExperimentalThemePlugin`'s generated
stylesheet — no manual `theme.css` link.
...
- token layer: generated by `ExperimentalThemePlugin` (vendored Radix scales
  in the package's `radix_colors/`; the old static `assets/theme.css` copy was
  removed once generation landed).
...
CSS but not yet exercised by the harness, and all cases run in light mode.

## Re-verified against the generated theme (2026-07-06)

After `ExperimentalThemePlugin` switched from a static `theme.css` to
compile-time generation from vendored Radix scale data, the full harness was
re-run with tokens coming exclusively from the plugin:

- **violet/slate (default): 5541/5541 (100%)** — identical to the static-theme
  baseline (an A/B against the old stylesheet showed zero delta).
- **iris/sand: 5541/5541 (100%)** — the harness's `rx.theme`/`color_scheme`
  flipped to iris/sand and the plugin configured to match; first verification
  on a non-default theme, validating the vendored scales and the
  `--accent-*`/`--gray-*` aliasing against Radix's own `data-accent-color`/
  `data-gray-color` mapping.
