...
def test_astro_page_template_app_mode_uses_client_load():
    """app-mode mounts the page-root island with `client:load`.

    The shared runtime is now SSR-safe (router adapter returns sane
    defaults when ``window`` is undefined; React Contexts default to
    empty objects) so Astro server-renders the page to real HTML at
    build time and React hydrates on page load.
    """
...
def test_astro_page_root_island_template_prerenders_by_default():
    """App-mode PageRoot must render during Astro SSR.

    Returning null until useEffect runs turns app mode into a pure client
    mount, which loses the prerendered HTML benefit Astro should preserve.
    """
    src = astro_page_root_island_template(page_module_import="../page.jsx")
    ...
    assert "return <Page />;" in src


def test_astro_page_root_island_template_wraps_app_layout():
    """App/static PageRoot modules can reuse the app-level provider layout."""
    src = astro_page_root_island_template(
        page_module_import="../page.jsx",
        app_layout_import="../../../../app/root.jsx",
    )
    assert "import { Layout as ReflexAppLayout }" in src
    assert "return <ReflexAppLayout><Page /></ReflexAppLayout>;" in src
...
def test_emit_astro_artifacts_aggregates_layout_config_and_pages():
    """``emit_astro_artifacts`` ships layout + config + per-route pages.

    Per-route ``PageRoot.tsx`` modules are emitted separately by the
    compile pipeline (which has the path information needed to resolve
    the import back to the underlying React module).
    """
    pages = [
        AstroEmitterInput(
            route="/",
    ...
    # ``emit_astro_artifacts`` is layout + config + pages only.
    assert not any("PageRoot.tsx" in p for p in paths)
