...
async def _self_chaining_superseding_handler(value: str = "default"):
    """A superseding handler that chains a second invocation of itself once.
    ...
    if value == "first":
        ctx = EventContext.get()
        await ctx.enqueue(
            Event.from_event_type(self_chaining_superseding_event("second"))[0]
        )
...
async def test_superseding_tracking_cleaned_up(
    processor: EventProcessor,
    token: str,
):
    """Supersession tracking does not leak once chains complete naturally.
    ...
    async with processor as ep:
        first = await ep.enqueue(
            token, Event.from_event_type(superseding_root_event("one"))[0]
        )
        await asyncio.wait_for(first.wait_all(), timeout=1)
        await _drain_superseded(ep)
        assert ep._superseded == {}

        second = await ep.enqueue(
            token, Event.from_event_type(superseding_root_event("two"))[0]
        )
    ...
    assert _CALL_LOG == [{"value": "one"}, {"value": "two"}]


async def test_self_chaining_superseding_handler_not_cancelled(
    processor: EventProcessor,
    token: str,
):
    """A superseding handler chaining itself does not cancel its own chain.
    ...
    processor.configure()
    async with processor as ep:
        fut = await ep.enqueue(
            token,
            Event.from_event_type(self_chaining_superseding_event("first"))[0],
        )
        await asyncio.wait_for(fut.wait_all(), timeout=1)
        await asyncio.wait_for(ep.join(), timeout=1)

    assert _CALL_LOG == [{"value": "first"}, {"value": "second"}]
